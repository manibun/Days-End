...
function ISTilesPickerTilesList:render()
	...
	for row=1,32 do
		for col=1,8 do
			...
			-- print(texture)
			if alltilesname[row]==nil then
				alltilesname[row]={}
				
			end
			alltilesname[row][col]=tileName
			if texture then
...
--]]
	...
	if col >= 0 and col < 8 and row>=0 and row<32 then
		self:drawRectBorder(xIndent + col * texW, yIndent + row * texH, texW, texH, 0.5, 1, 1, 1)
		-- print(alltilename[row+1][col+1])
		if self.mouseDown and not isKeyDown(Keyboard.KEY_L) then
			BetterTilePickertiles=alltilesname[row+1][col+1]
...
function ISTilesPickerDebugUI:populateList()
	...
	for i=1,#BTPAlltiles do
		local name = BTPAlltiles[i]
...
function ISTilesPickerTilesList:onMouseUp(x, y)
...
function ISTilesPickerTilesList:onMouseDown(x, y)
	self.mouseDown = true
	-- print("onMouseDown")
end
