...
function ISVehicleMechanics:doPartContextMenu(part, x,y)
	...
	if part:getId() == "Engine" and not VehicleUtils.RequiredKeyNotFound(part, self.chr) then
		if part:getCondition() > 10 and self.chr:getPerkLevel(Perks.Mechanics) >= 10 and self.chr:getInventory():contains("Wrench") then
...
function ISVehicleMechanics:doMenuTooltip(part, option, lua, name)
	...
	if lua == "takeengineparts" then
		...
		rgb = " <RGB:1,1,1>";
		if self.chr:getPerkLevel(Perks.Mechanics) < 10 then
			rgb = " <RGB:1,0,0>";
		end
		tooltip.description = tooltip.description .. rgb .. getText("IGUI_perks_Mechanics") .. " " .. self.chr:getPerkLevel(Perks.Mechanics) .. "/" .. 10 .. " <LINE>";
		rgb = " <RGB:1,1,1>";
